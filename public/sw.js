...
function shouldCacheExerciseImage(url) {
  const isLocalExerciseImage =
    url.origin === self.location.origin && url.pathname.startsWith('/images/exercises/')
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    (async () => {
      ...
      await Promise.all(
        cacheNames
          .filter((cacheName) => cacheName.startsWith('workout-tracker-') && !ACTIVE_CACHES.has(cacheName))
