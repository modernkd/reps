...
import {
  ExerciseHistoryView,
  type ExerciseViewFilters,
} from "@/components/ExerciseHistoryView";
...
function applyTheme(theme: ThemeMode) {
  ...
  if (themeMeta) {
    themeMeta.setAttribute(
      "content",
      theme === "dark" ? themeMetaDark : themeMetaLight,
    );
  }
}
...
function WorkoutDashboard() {
  ...
  const view = (
    search.view === "history" ? "exercises" : (search.view ?? "calendar")
  ) as DashboardView;
    ...
    navigate({
      search: (prev) => {
        ...
        const normalizedView =
          next.view === "history" ? "exercises" : next.view;
        ...
        return {
          view: undefined,
          month:
            calendarMonth === getDefaultMonth(workouts)
              ? undefined
              : calendarMonth,
  ...
  const [modalState, setModalState] = useState<
    { mode: "create"; date: string } | { mode: "edit"; workout: Workout } | null
  >(null);
  ...
  const activeTemplateId = useMemo(() => {
    ...
    if (
      search.template &&
      templates.some((template) => template.id === search.template)
    ) {
  ...
  const visibleSessions = useMemo(() => {
    ...
    return sessions.filter(
      (session) =>
        session.templateId === activeTemplateId ||
        session.templateId === MANUAL_TEMPLATE_ID,
    );
  ...
  const canClearAfter = useMemo(
    () =>
      visibleSessions.some(
        (session) =>
          session.status !== "completed" && session.date > selectedDate,
      ),
  ...
  useEffect(() => {
    ...
    const selectedSession = sessions.find(
      (session) => session.id === search.session,
    );
  ...
  const handleFilterChange = (nextTypeIds: string[]) => {
    const encoded =
      nextTypeIds.length > 0 ? nextTypeIds.sort().join(",") : undefined;
  ...
  const handleDeleteTemplate = async (templateId: string) => {
    ...
    const confirmed = window.confirm(
      copy.route.confirmDeleteTemplate(target.name),
    );
    if (!confirmed) {
      return;
    }
    ...
    try {
      ...
      if (
        ...
        error.message === LAST_TEMPLATE_DELETE_ERROR
      ) {
        setErrorMessage(copy.route.minTemplatesRequired);
        return;
      }
  ...
  const saveWorkout = async (value: WorkoutFormValue) => {
    ...
    try {
      ...
      trackEvent("workout_save_success", {
  ...
  const handleClearAfter = async (date: string) => {
    ...
    try {
      const { sessionsDeleted, workoutsDeleted } =
        await clearDataAfterDate(date);
  ...
  const handleClearBefore = async (date: string) => {
    ...
    try {
      const { sessionsDeleted, workoutsDeleted } =
        await clearDataBeforeDate(date);
      const selectedSession = search.session
        ? sessions.find((session) => session.id === search.session)
        : undefined;
      const previewSession = previewSessionId
  ...
  const handleCopyToNextWeek = async (workout: Workout) => {
    ...
    try {
      if (workout.scheduledSessionId) {
        await duplicateScheduledSession(
          workout.scheduledSessionId,
          nextWeekDate,
        );
      } else {
        await planManualWorkout({
  ...
  const handleSkipSession = async (sessionId: string) => {
    ...
    try {
      ...
      if (shouldMoveToTomorrow) {
        try {
          const movedSessionId = await moveScheduledSessionToDate(
            sessionId,
            nextDate,
          );
          setSearch((prev) => ({
  ...
  const handleCompleteSession = async (
    summary: Parameters<typeof completeGuidedSession>[0]["summary"],
    notes?: string,
  ) => {
    if (!activeSession) {
      return;
  ...
  const templateSource =
    templateModalState?.mode === "duplicate" &&
    templateModalState.sourceTemplateId
      ? templates.find(
          (template) => template.id === templateModalState.sourceTemplateId,
        )
      : templateModalState?.mode === "edit"
        ? templates.find(
            (template) => template.id === templateModalState.templateId,
          )
        : undefined;
  ...
  return (
    <main className={styles.page}>
      <div className={styles.container}>
        ...
        {showLoading ? (
          <p className={styles.loading}>{copy.route.loadingWorkouts}</p>
        ) : null}
        ...
        {activeSession && activeDraft ? (
          <section
            className={
              isWorkoutMode ? styles.workoutModeSurface : styles.floatingCard
            }
          >
            <GuidedWorkoutView
              language={language}
        ...
        {search.session && !activeDraft ? (
          <section
            className={isWorkoutMode ? styles.workoutModeError : styles.error}
          >
      ...
      >
        {modalState ? (
          <WorkoutForm
            language={language}
            initialValue={
              modalState.mode === "edit" ? modalState.workout : undefined
            }
            defaultDate={
              modalState.mode === "create" ? modalState.date : selectedDate
            }
            types={workoutTypes}
