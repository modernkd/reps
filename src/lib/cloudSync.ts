...
export async function signInCloudUser(email: string, password: string): Promise<void> {
  const client = getSupabaseClient()
  if (!client) {
...
export async function signInCloudUserWithGoogle(redirectTo?: string): Promise<void> {
  const client = getSupabaseClient()
  if (!client) {
    throw new Error('Cloud sync is not configured.')
  }
