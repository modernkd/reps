...
describe("exerciseDb", () => {
  ...
  describe("searchExercises", () => {
    it("searches by name substring", async () => {
      ...
      expect(results.some((r) => r.name.toLowerCase().includes("bench"))).toBe(
        true,
      );
    });
    ...
    it("searches by primary muscle", async () => {
      ...
      expect(results.length).toBeGreaterThan(0);
      expect(results.some((r) => r.primaryMuscles?.includes("chest"))).toBe(
        true,
      );
    });
  ...
  describe("getExercisesByMuscle", () => {
    it("returns exercises for chest", async () => {
      ...
      expect(results.length).toBeGreaterThan(0);
      expect(results.every((r) => r.primaryMuscles?.includes("chest"))).toBe(
        true,
      );
    });
    ...
    it("returns exercises for biceps", async () => {
      ...
      expect(results.length).toBeGreaterThan(0);
      expect(results.every((r) => r.primaryMuscles?.includes("biceps"))).toBe(
        true,
      );
    });
  ...
  describe("getExerciseSwapRecommendations", () => {
    it("returns ranked similar alternatives by muscle/equipment/category", async () => {
      ...
      expect(
        result.similar.every(
          (option) => option.exercise.id !== result.sourceExercise?.id,
        ),
      ).toBe(true);
      expect(
        result.similar.some(
          (option) =>
            option.muscleFocus.length > 0 ||
            option.sameEquipment ||
            option.sameCategory,
        ),
      ).toBe(true);
    });
    ...
    it("returns beginner-safe alternatives with beginner level only", async () => {
      ...
      expect(
        result.beginnerSafe.every(
          (option) => option.exercise.level === "beginner",
        ),
      ).toBe(true);
    });

    it("extracts concise technique snippets from matched exercise instructions", async () => {
      const result = await getExerciseSwapRecommendations(
        "Barbell Bench Press - Medium Grip",
      );
