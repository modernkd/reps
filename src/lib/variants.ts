...
export function isCustomExerciseVariant(
  exerciseId: string,
  exerciseName: string,
): boolean {
  ...
  return !knownVariants.some(
    (variant) => normalizeVariant(variant) === normalizedName,
  );
}
...
export function getCatalogExerciseIdByName(
  exerciseName: string,
): string | null {
