...
async function clearSessionDraftCollection() {
  ...
  const keys = activeSessionDraftsCollection.toArray.map(
    (item) => item.sessionId,
  );
  if (keys.length === 0) {
    return;
...
describe("template and scheduling", () => {
  ...
  it("creates a custom template and schedules only from the selected start date", async () => {
    ...
    expect(planTemplatesCollection.get(template.id)?.startDate).toBe(
      "2026-02-10",
    );
  ...
  it("updates a template and rebuilds scheduled sessions", async () => {
    ...
    expect(planTemplatesCollection.get(template.id)?.name).toBe(
      "Updated cycle",
    );
  ...
  it("deletes a template and related plan entities", async () => {
    ...
    expect(
      planDaysCollection.toArray.filter(
        (day) => day.templateId === template.id,
      ),
    ).toHaveLength(0);
    expect(
  ...
  it("plans a manual workout with the provided details", async () => {
    ...
    expect(planDaysCollection.get(MANUAL_PLAN_DAY_ID)?.label).toBe(
      "Planned workout",
    );
  ...
  it("duplicates a planned session for a later date", async () => {
    ...
    const duplicated = await duplicateScheduledSession(
      session.id,
      "2026-02-28",
    );
    ...
    expect(sessionPlansCollection.get(duplicated.id)?.title).toBe(
      "Custom plan",
    );
    expect(sessionPlansCollection.get(duplicated.id)?.notes).toBe(
      "Keep accessories",
    );
  });
  ...
  it("completes manual sessions using planned workout values", async () => {
    ...
    expect(scheduledSessionsCollection.get(session.id)?.status).toBe(
      "completed",
    );
  });
});
...
describe("clearDataAfterDate", () => {
  it("clears uncompleted sessions after the selected day and preserves completed history", async () => {
    ...
    expect(
      workoutsCollection.get("workout-detached")?.scheduledSessionId,
    ).toBeUndefined();

    expect(scheduledSessionsCollection.has("session-future-completed")).toBe(
      true,
    );
    expect(scheduledSessionsCollection.has("session-future-planned")).toBe(
      false,
    );
    expect(scheduledSessionsCollection.has("session-past")).toBe(true);

    expect(activeSessionDraftsCollection.has("session-future-planned")).toBe(
      false,
    );
  ...
  it("prevents schedule regeneration after the cleared date", async () => {
    ...
    expect(
      scheduledSessionsCollection.toArray.some(
        (session) => session.date > "2026-02-10",
      ),
    ).toBe(false);
  });
...
describe("clearAllUncompletedSessions", () => {
  it("removes every non-completed scheduled session and cleans linked records", async () => {
    ...
    expect(
      workoutsCollection.get("workout-planned")?.scheduledSessionId,
    ).toBeUndefined();
    expect(
      workoutsCollection.get("workout-in-progress")?.scheduledSessionId,
    ).toBeUndefined();
    expect(
      workoutsCollection.get("workout-skipped")?.scheduledSessionId,
    ).toBeUndefined();
    expect(
      workoutsCollection.get("workout-completed")?.scheduledSessionId,
    ).toBe("session-completed");
    expect(activeSessionDraftsCollection.has("session-in-progress")).toBe(
      false,
    );
  ...
  it("returns zero when there are no non-completed sessions", async () => {
    ...
    expect(scheduledSessionsCollection.has("session-completed-only")).toBe(
      true,
    );
  });
});
...
describe("clearDataBeforeDate", () => {
  it("clears uncompleted sessions before the selected day and preserves completed history", async () => {
    ...
    expect(scheduledSessionsCollection.has("session-past-completed")).toBe(
      true,
    );
    ...
    expect(activeSessionDraftsCollection.has("session-past-planned")).toBe(
      false,
    );
