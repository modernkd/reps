...
describe("selectors", () => {
  ...
  it("aggregates weekly trend series", () => {
    ...
    const firstWorkoutWeek = series.points.find(
      (point) => point.weekStart === "2026-02-02",
    );
  ...
  it("filters average weight trend by workout type", () => {
    ...
    const liftWeek = liftOnly.points.find(
      (point) => point.weekStart === "2026-02-02",
    );
    const runWeek = runOnly.points.find(
      (point) => point.weekStart === "2026-02-02",
    );
  ...
  it("builds exercise progress summaries ordered by latest log date", () => {
    ...
    expect(bench?.logs[0]).toEqual({
      date: "2026-02-10",
      weightKg: 120,
      setsLogged: 1,
    });
    expect(bench?.weightPoints.at(-1)).toEqual({
      date: "2026-02-10",
      value: 120,
    });
  });
  ...
  it("includes available exercises that have never been logged", () => {
    ...
    const lateralRaises = summaries.find(
      (entry) => entry.key === "lateral raises",
    );
