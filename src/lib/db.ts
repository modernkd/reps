...
function normalizeWorkoutDataSnapshot(
  input: Partial<WorkoutDataSnapshot>,
): WorkoutDataSnapshot {
  return {
    schemaVersion:
    ...
    exportedAt:
      typeof input.exportedAt === "string" && input.exportedAt.length > 0
        ? input.exportedAt
        : nowIso(),
    workoutTypes: sortById(
      parseSnapshotArray(input.workoutTypes, workoutTypeSchema),
    ),
    workouts: sortById(parseSnapshotArray(input.workouts, workoutSchema)),
    planTemplates: sortById(
      parseSnapshotArray(input.planTemplates, planTemplateSchema),
    ),
    ...
    sessionPlans: sortById(
      parseSnapshotArray(input.sessionPlans, sessionPlanSchema),
    ),
  };
}
...
export function serializeWorkoutDataSnapshot(
...
export async function replaceWorkoutDataSnapshot(
  ...
  if (normalized.exerciseTemplates.length > 0) {
    await persist(
      exerciseTemplatesCollection.insert(normalized.exerciseTemplates),
    );
  }
  ...
  if (normalized.scheduledSessions.length > 0) {
    await persist(
      scheduledSessionsCollection.insert(normalized.scheduledSessions),
    );
  }
  if (normalized.activeSessionDrafts.length > 0) {
    await persist(
      activeSessionDraftsCollection.insert(normalized.activeSessionDrafts),
    );
  }
...
export async function ensureDefaultWorkoutTypes(): Promise<void> {
  const existingIds = new Set(
    workoutTypesCollection.toArray.map((item) => item.id),
  );
  const missing = DEFAULT_WORKOUT_TYPES.filter(
...
export async function ensureDefaultExerciseCatalog(): Promise<void> {
  const existingNames = new Set(
    exerciseCatalogCollection.toArray.map((entry) =>
...
export async function addExerciseCatalogEntry(
  name: string,
...
export async function deleteWorkout(
  ...
  if (workout?.scheduledSessionId) {
    ...
    if (linkedSession) {
      ...
      await persist(scheduledSessionsCollection.delete([linkedSession.id]));
    }
  }
}
...
export async function clearAllUncompletedSessions(): Promise<{
  ...
  await persist(
    scheduledSessionsCollection.delete(Array.from(sessionIdsToDelete)),
  );
...
export async function applyTemplateToCalendar(input: {
  ...
  if (input.startPlanDayId) {
    const startPlanDay = templateDays.find(
      (day) => day.id === input.startPlanDayId,
    );
    if (!startPlanDay) {
    ...
    const startDateWeekday =
      ((parseISO(normalizedStartDate).getDay() + 6) % 7) + 1;
...
export async function planManualWorkout(
  input: {
    date: string;
  } & PlannedWorkout,
...
export async function duplicateScheduledSession(
  ...
  const nextId = scheduledSessionId(
    session.templateId,
    session.planDayId,
    nextDate,
  );
...
export async function completeGuidedSession(input: {
  ...
  const workout = await addWorkout({
    ...
    durationMin:
      plannedWorkout?.durationMin ??
      estimateDurationFromSetLogs(exercises, input.summary),
...
function toSessionExercisePlan(
