...
export function ExerciseHistoryView({
  ...
  const [remoteExerciseSuggestions, setRemoteExerciseSuggestions] = useState<string[]>([])
  ...
  }, [availableExerciseNames, remoteExerciseSuggestions, enhancedSearchResults])
  ...
  const renderAddExerciseForm = () => (
    <form className={styles.addExerciseForm} onSubmit={handleSubmitExercise}>
      <div className={styles.addExerciseRow}>
        ...
        />
        <button type="submit" disabled={isAddingExercise || newExerciseName.trim().length === 0}>
          {isAddingExercise ? copy.historyView.addingExercise : copy.historyView.addExercise}
        </button>
      </div>
    </form>
  )
  ...
  const formattedItems = entries.map((entry, index) => {
    ...
    const referenceImages =
      referenceContent?.images.length
    ...
    return {
      ...
      subtitle: entry.lastRecordedAt
        ? copy.historyView.lastRecorded(
            format(parseISO(entry.lastRecordedAt), 'MMM d, yyyy', { locale: dateLocale }),
          )
        : copy.historyView.neverRecorded,
      handle: entry.totalLogs > 0 ? copy.historyView.sessionCount(entry.totalLogs) : undefined,
      location:
          ...
          onCardClick={(item) => {
            ...
            if (entry) {
              navigate({
                ...
                search: selectedTypeIds.length > 0 ? { types: selectedTypeIds.join(',') } : {},
              })
            }
          }}
        />
