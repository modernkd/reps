...
async function captureStep(
  ...
  const path = testInfo.outputPath(
    `${viewportName}-${String(step).padStart(2, "0")}-${label}.png`,
  );
  await page.screenshot({ path });
  await testInfo.attach(
    `${viewportName}-${String(step).padStart(2, "0")}-${label}`,
    {
      path,
...
for (const viewport of VIEWPORTS) {
  test.describe(`responsive interactions (${viewport.name})`, () => {
    ...
    test(`captures key UI states in ${viewport.name}`, async ({
      page,
    }, testInfo) => {
      ...
      await expect(
        page.getByRole("heading", { name: "Consistency dashboard" }),
      ).toBeVisible();
      ...
      const manageTemplateButton = page
        .locator('summary[aria-label^="Manage "]')
        .first();
      ...
      await expect(
        page.getByRole("dialog", { name: "Edit template" }),
      ).toBeVisible();
      await captureStep(
        page,
        testInfo,
      ...
      await page
        .getByRole("button", { name: "Add template to calendar" })
        .click();
      await expect(
        page.getByRole("dialog", { name: "Apply template to calendar" }),
      ).toBeVisible();
      await captureStep(
        page,
        testInfo,
      ...
      await expect(
        page.getByRole("heading", { name: "Weekly Workout Trends" }),
      ).toBeVisible();
      ...
      await dayCell.click();
      await expect(
        page.getByRole("region", { name: "Workout details for selected date" }),
      ).toBeVisible();
