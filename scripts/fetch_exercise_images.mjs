...
async function run() {
  ...
  for (const [id, exactName] of Object.entries(map)) {
    ...
    if (!match) {
      console.log(`[!!] Could not find exercise matching exactly: ${exactName}`);
      continue;
    }
    ...
    if (match.images && match.images.length > 0) {
      ...
      for (let i = 0; i < imagesToFetch.length; i++) {
        ...
        try {
          ...
          const jpgDest = path.join(imagesDir, `${id}_${i}${path.extname(imgPath)}`);
          if (fs.existsSync(jpgDest)) fs.unlinkSync(jpgDest);
        } catch (err) {
          console.error(`[!!] Failed downloading/converting ${url}:`, err.message);
        }
      }
    } else {
  ...
  const dataDest = path.join(process.cwd(), 'src', 'lib', 'defaultExercisesData.ts');
